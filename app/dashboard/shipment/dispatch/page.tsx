...
import { ArrowLeft, ArrowRight, PackageCheck, Loader2, Calendar, User, Truck, Send } from "lucide-react";
...
export default function ShipmentDispatchPage() {
    ...
    const fetchDispatchableOrders = async () => {
        try {
    ...
    const handleSelectOrder = (id: string) => {
        router.push(`/dashboard/shipment/dispatch/detail?id=${id}`);
    };
    ...
    return (
        <div className="w-full h-full min-h-[80vh] flex flex-col items-center justify-center p-6 animate-in slide-in-from-right-8 duration-500">
            <div className="w-full max-w-4xl text-center">
                ...
                <p className="text-gray-500 mb-10 text-lg">Select a picked order to confirm it has been loaded onto the truck</p>
                ...
                <div className="bg-white p-6 rounded-[32px] shadow-sm border border-gray-100 min-h-[400px] flex flex-col text-left">
                    <div className="flex justify-between items-center px-4 mb-6">
                        ...
                        <span className="bg-indigo-100 text-indigo-800 px-3 py-1 rounded-full font-black text-sm">{orders.length}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {loading ? (
                            ...
                            <div className="col-span-full h-64 flex flex-col items-center justify-center text-gray-400 gap-3 border-2 border-dashed border-gray-200 rounded-2xl">
                                ...
                                <p className="font-bold text-lg text-gray-400">No orders ready for dispatch.</p>
                            </div>
                        ) : (
                            orders.map(order => (
                                <div
                                    ...
                                    className="group bg-gray-50 hover:bg-indigo-50 border border-gray-100 hover:border-indigo-200 p-5 rounded-2xl cursor-pointer transition-all flex flex-col gap-3 relative overflow-hidden"
                                >
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <span className="font-black text-xl text-gray-900 group-hover:text-indigo-700">{order.outbound_id}</span>
                                        ...
                                        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-sm opacity-0 group-hover:opacity-100 transition-opacity translate-x-4 group-hover:translate-x-0">
                                            <ArrowRight className="w-5 h-5 text-indigo-600 stroke-[3px]" />
                                        </div>
                                    </div>

                                    <div className="h-px w-full bg-gray-200 my-1 group-hover:bg-indigo-100" />

                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-start gap-2 text-sm text-gray-600">
                                            <User className="w-4 h-4 shrink-0 mt-0.5" />
                                            <span className="font-bold line-clamp-1">{order.customer?.name || "No Customer Assigned"}</span>
                                        </div>
                                        <div className="flex items-start gap-2 text-xs text-gray-500">
                                            <Truck className="w-4 h-4 shrink-0 mt-0.5" />
                                            <span className="font-medium line-clamp-1">{order.shipment?.license_plate || "No Truck Assigned"}</span>
                                        </div>
                                    </div>
                                </div>
                            ))
